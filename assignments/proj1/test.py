...
def check1_5(fertility_over_time, mode=CHECK):
    ...
    if not check_table_labels(usa2010, ('Year', 'Children per woman'), mode):
        pass
    elif not np.array_equal(usa2010.sort('Year').column('Year'), np.arange(2010, 2018+1)):
...
def check1_11(stats_for_year, mode=CHECK):
    ...
    if not check_table_labels(stats1990, ('geo', 'population_total', 'children_per_woman_total_fertility', 'child_mortality_under_5_per_1000_born'), mode):
        ...
        if not is_close(yem_stats, [  1.2057039e+07, 8.6000000e+00, 1.2580000e+02]).all():
...
def check2_2(recent, mode=CHECK):
    ...
    elif recent.where('geo', 'ago').column('poverty_total').item(0) != 7034108:
        fails('Incorrect. The number of people in extreme poverty in Angola should be 7,034,108. Did you round?', mode)
    else:
        ok(mode)
