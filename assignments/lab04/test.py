...
def check1_1(total_pay_type, mode=CHECK):
    if total_pay_type == 1:
        ...
        check_multiple_choice_in_bounds(total_pay_type, 3, mode)
...
def check4_3(dist_shape, mode=CHECK):
    if dist_shape == 2:
        correct(mode)
    else:
        check_multiple_choice_in_bounds(dist_shape, 4, mode)
