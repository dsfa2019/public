...
def check1_1_2(stemmed_message, mode=CHECK):
    ...
    elif not len(stemmed_message) < len('changing'):
        fails("Incorrect. `stemmed_message` should be a substring of 'changing'", mode)
    elif not 'changing'.startswith(stemmed_message):
        fails("Incorrect. `stemmed_message` should be a substring of 'changing'", mode)
    else:
        ok(mode)
...
def check1_1_3(unstemmed_run, mode=CHECK):
    ...
    elif not unstemmed_run.item(0).startswith('learn'):
        fails("Incorrect. All items should start with the substring `learn`.", mode)
...
def check2_1_3(distance_from_batman_returns, mode=CHECK):
    ...
    elif not np.isclose(distance_from_batman_returns('titanic'), 0.0023550197398520892):
        fails("Incorrect.", mode)
    else:
        ok(mode)
